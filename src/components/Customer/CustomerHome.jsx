...
import { StyleSheet, Text, View, Animated, Pressable, TouchableOpacity, Platform, StatusBar } from 'react-native';
...
function AnimatedTabLabel({ label, color, focused }) {
  ...
  return (
    <Animated.Text style={[styles.tabLabel, { color, transform: [{ scale: scaleAnim }], opacity: opacityAnim }]}>
      {label}
    </Animated.Text>
  );
}
...
const CustomerHome = ({ route, navigation }) => {
  ...
  const greetingEmoji = '👋';
  const userInitial = (email?.trim()?.charAt(0) || 'C').toUpperCase();
        ...
        <View style={[styles.topBarWrap, { paddingTop: topInset + 8 }]}>
          <View style={styles.topBar}>
            <View style={styles.topBarLeft}>
              <Text style={styles.topBarCaption}>Customer</Text>
              <Text style={styles.brand}>Apna Bazar</Text>
              <Text style={styles.greeting} numberOfLines={1}>
                {greetingEmoji} Hello{email ? `, ${email}` : ''}
              </Text>
            </View>
            <View style={styles.badgeShadow}>
              <TouchableOpacity activeOpacity={0.8} onPress={async () => {
                try {
                  ...
                  let userRes = await axios.post(`${Config.API_URL}/getprofile`, { token })
      ...
      <Tab.Navigator
        ...
        screenOptions={({ route }) => ({
          ...
          tabBarIcon: ({ color, focused }) => (
            <AnimatedTabBarIcon name={tabIcons[route.name]} color={color} focused={focused} />
          ),
        })}
      >
